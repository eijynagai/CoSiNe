...
def write_membership_list_to_file(file_name, membership):
    with open(file_name, "w") as f:
        f.write(
            "\n".join(
                str(i) + " " + str(membership[i] + 1) for i in range(len(membership))
            )
        )
...
def clustering_statistics(net, membership, show_cluster_size_dist=False):
    ...
    if show_cluster_size_dist:
        print(cluster_sizes)
    print(
        "min, max, mean, median cluster sizes:",
        min_size,
        max_size,
        mean_size,
        median_size,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Estimating properties of a network/clustering pair."
    )
    parser.add_argument(
        "-n", metavar="net", type=str, required=True, help="network edge-list path"
    )
    parser.add_argument(
        "-c",
        metavar="clustering",
    ...
    (
        ...
        modularity_score,
        coverage,
    ) = clustering_statistics(net, membership)
    ...
    print(
        "tau1, xmin1, tau2, xmin2 [fixed xmin]",
        tau1_fixed,
        xmin1_fixed,
        tau2_fixed,
        xmin2_fixed,
