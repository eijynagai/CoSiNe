...
def gen_lfr(stats_path, lfr_path, cmin):
    ...
    cmd = (
        ...
        + lfr_path
        + " -N "
        + str(net_cluster_stats["node-count"])
        + " -k "
        + str(net_cluster_stats["mean-degree"])
        + " -maxk "
        + str(net_cluster_stats["max-degree"])
        + " -mu "
        + str(net_cluster_stats["mixing-parameter"])
        + " -maxc "
        + str(net_cluster_stats["max-cluster-size"])
        + " -minc "
        + cmin
        + " -t1 "
        + str(net_cluster_stats["tau1"])
        + " -t2 "
        + str(net_cluster_stats["tau2"])
    )
    print(cmd)
    os.system(cmd)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Emulating real networks using LFR graphs."
    )
    parser.add_argument(
        "-n",
        metavar="net",
    ...
    parser.add_argument(
        "-cm", metavar="cmin", type=str, required=False, help="Minimum community size"
    )
