...
def run_spinglass(graph, resolution=None):
    ...
    # Optionally assign weights if present
    # Default spinglass ignores weights; adjust if needed
    ct = ig_graph.community_spinglass()
    membership = ct.membership
    # Map back to original node labels
    communities = {node: membership[i] for i, node in enumerate(nodes)}
    logging.info("Spinglass: found %d communities", len(ct))
    return communities
