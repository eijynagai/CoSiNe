...
def generate_signednetwork(
    community_size, num_communities, intra_edges, inter_edges, p1, p2, seed=None
):
    if seed is not None:
        np.random.seed(seed)
    ...
    for edge_index in np.random.choice(
        len(positive_edges), int(intra_edges * p1), replace=False
    ):
    ...
    for edge_index in np.random.choice(
        len(negative_edges), int(inter_edges * p2), replace=False
    ):
...
def visualize_top_weighted_nodes(graph, gene_name, top_n):
    ...
    edges = [
        (gene_id, neighbor, data["weight"]) for neighbor, data in graph[gene_id].items()
    ]
    ...
    labels = {
        n: graph.nodes[n]["name"] for n in subgraph.nodes()
    }  # ノードIDではなく遺伝子名でラベルを付ける

    node_colors = [
        "red" if node == gene_id else "lightblue" for node in subgraph.nodes()
    ]
...
def visualize_community(graph, partition, community_id):
    ...
    nx.draw_networkx_labels(
        subgraph, pos, labels={n: subgraph.nodes[n]["name"] for n in subgraph.nodes()}
    )
...
def visualize_top_weighted_nodes_between_genes(graph, gene_names, top_n):
    ...
    node_colors = [
        "red" if node in gene_ids else "lightblue" for node in subgraph.nodes()
    ]
...
def extract_subgraph(graph, partition, community_id):
    nodes_in_community = [
        node for node in graph.nodes if partition[node] == community_id
    ]
    subgraph = graph.subgraph(nodes_in_community)
    return subgraph
