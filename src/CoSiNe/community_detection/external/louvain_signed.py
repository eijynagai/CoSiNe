...
from CoSiNe.community_detection.external.network import (
    calc_entropy,
    generate_signednetwork,
)
...
def plot_alphas_distributions(G_positive, G_negative, resolution, seed):
    ...
    for alpha in alphas:
        partition = LouvainSigned(G_positive, G_negative).best_partition(
            alpha=alpha, resolution=resolution, seed=seed
        )
...
class GraphInfo:
    ...
    def _initialize_community_info(self, partition):
        ...
        self.linksum_dict = {
            node: self.get_linksum_dict(node, partition)
            for node in self.graph_whole.nodes()
        }
    ...
    def calc_modularity(self, partition, *, resolution=1.0):
        m = self.size
        modularity = 0.0
    ...
    def _remove_node(self, node, partition, linksum_dict):
        ...
        self.community_degrees[community] = self.community_degrees.get(
            community, 0.0
        ) - self.node_degrees.get(node, 0.0)
        self.community_sizes[community] = (
            self.community_sizes.get(community, 0.0)
            - linksum
            - self.loops.get(node, 0.0)
        )
    ...
    def _delta_q_1(self, node, linksum_dict, partition, resolution):
        ...
        m = self.size
        linksum = linksum_dict.get(community, 0.0)
    ...
    def _delta_q_2(self, node, linksum_dict, neighboring_community, resolution):
        ...
        m = self.size
        linksum = linksum_dict.get(neighboring_community, 0.0)
...
class LouvainSigned:
    def __init__(self, positive_graph, negative_graph, *, mode="positive"):
        ...
        self.ginfo_pos = GraphInfo(
            self.graph_whole, positive_graph, self.partition, self.weight_key
        )
        self.ginfo_neg = GraphInfo(
            self.graph_whole, negative_graph, self.partition, self.weight_key
        )
    ...
    def _modularity(self, partition, *, alpha=1.0, resolution=1.0):
    ...
    def _move_nodes(self, random_generator, *, alpha=1.0, resolution=1.0):
        modified = True
        nb_pass_done = 0
        ...
        while modified and nb_pass_done != ITER_LIMIT_PER_LOCALMOVE:
            ...
            for node in self._randomize(self.graph_whole.nodes(), random_generator):
                ...
                q1_pos = self.ginfo_pos._delta_q_1(
                    node, linksum_dict_pos, self.partition, resolution
                )
                q1_neg = self.ginfo_neg._delta_q_1(
                    node, linksum_dict_neg, self.partition, resolution
                )
                ...
                best_increase = 0
                for neighboring_community in self._randomize(
                    linksum_dict_pos.keys(), random_generator
                ):
                    q2_pos = self.ginfo_pos._delta_q_2(
                        node, linksum_dict_pos, neighboring_community, resolution
                    )
                    q2_neg = self.ginfo_neg._delta_q_2(
                        node, linksum_dict_neg, neighboring_community, resolution
                    )

                    #                    delta_Q = alpha * (q1_pos + q2_pos) - (1 - alpha) * (1 - q1_neg - q2_neg)
                    delta_Q = alpha * (q1_pos + q2_pos) - (1 - alpha) * (
                        q1_neg + q2_neg
                    )
                ...
                self.ginfo_pos._insert_node(
                    node, self.partition, linksum_dict_pos, best_community
                )
                self.ginfo_neg._insert_node(
                    node, self.partition, linksum_dict_neg, best_community
                )
    ...
    def generate_dendrogram(self, mode, random_generator, *, alpha=1.0, resolution=1.0):
        ...
        while True:
            ...
            current_graph_pos = self._aggregate_nodes(
                renumberd_partition, current_graph_pos
            )
            current_graph_neg = self._aggregate_nodes(
                renumberd_partition, current_graph_neg
            )
            self.graph_whole = self.get_signed_graph(
                current_graph_pos, current_graph_neg, mode
            )

            self.partition = {
                node: i for i, node in enumerate(self.graph_whole.nodes())
            }
            self.ginfo_pos = GraphInfo(
                self.graph_whole, current_graph_pos, self.partition, self.weight_key
            )
            self.ginfo_neg = GraphInfo(
                self.graph_whole, current_graph_neg, self.partition, self.weight_key
            )
            new_Q = self._modularity(self.partition, alpha=alpha, resolution=resolution)
    ...
    def best_partition(self, *, alpha=1.0, resolution=1.0, seed=None):
        if not isinstance(alpha, (int, float)):
            print("Error: Alpha value must be a number. Please provide a valid value.")
            return
        elif not 0 <= alpha <= 1:
            print(
                "Error: Alpha value must be in the range [0, 1]. Please provide a valid value."
            )
            return
        ...
        print(
            f"Final Modularity: {self._modularity(partition, alpha=alpha, resolution=resolution)}"
        )
        print(
            f"Positive Modularity (Q+): {self.ginfo_pos.calc_modularity(partition, resolution=resolution)}"
        )
        print(
            f"Negative Modularity (Q-): {self.ginfo_neg.calc_modularity(partition, resolution=resolution)}"
        )
...
def main():
    ...
    parser.add_argument(
        "--alpha",
        help="alpha parameter (from 0 to 1, default: 0.5)",
        type=float,
        default=0.5,
    )
    parser.add_argument(
        "--resolution",
        help="resolution for louvain (default: 1.0)",
        type=float,
        default=1.0,
    )
    ...
    G_positive, G_negative = generate_signednetwork(
        community_size, num_communities, intra_edges, inter_edges, p1, p2
    )
    ...
    print(
        f"G: Nodes={nx.compose(G_positive, G_negative).number_of_nodes()}, Edges={nx.compose(G_positive, G_negative).number_of_edges()}"
    )

    graph = LouvainSigned(G_positive, G_negative, alpha, seed=args.seed)
    partition = graph.best_partition()
